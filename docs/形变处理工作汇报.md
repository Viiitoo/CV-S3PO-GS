...
| **形变点选择** | $\text{deform\_mask} = \text{max}(\text{deformation\_accum}) > \text{threshold}$<br>默认 threshold = 0.01 | $\text{deform\_mask} = \text{max}(\text{deformation\_accum}) / 100 > \text{threshold}$<br>默认 threshold 可配置 | EH-SurGS额外除以100进行归一化 |
...
**当前工作区**：
- 直接使用累积值，没有除以100的归一化步骤
- 公式：`max_deform = _deformation_accum`
- 阈值判断：`_deformation_table = max_deform > threshold`

```1486:1489:gaussian_splatting/scene/gaussian_model.py
...
| **归一化** | 除以100 | 无归一化 | EH-SurGS的阈值需要更大（因为累积值更大） |
...
**建议**：
- 如果需要捕捉长期形变趋势，建议采用EH-SurGS的累加方式
- 如果内存受限或只关注瞬时形变，当前工作区的方式更合适
- 如果采用EH-SurGS方式，需要在`update_deformation_table`中添加除以100的归一化步骤
...
```1463:1500:gaussian_splatting/scene/gaussian_model.py
    @torch.no_grad()
